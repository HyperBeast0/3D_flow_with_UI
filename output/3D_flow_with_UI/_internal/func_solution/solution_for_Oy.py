...
def circle_calculate_slice_Oy_explicit(grid_x, grid_y, coord_x, coord_y, radius):
    # Константы
    ...
    def solve_flow(u, v, p, dt, nt):
        for i in range(nt):
            ...
            Poisson = rho * (
                    ...
                    (2 * dx) + (v_new[2:, 1:-1] - v_new[:-2, 1:-1]) / (2 * dy)) / dt
            ...
            for i in range(0, nx):
                u[i, 0] = u[i, -1] = U - i * dt_U  # Граниченое условие слева и справа
                u[0, :] = U  # Граниченое условие сверху
    ...
    def plot_flow(u, v, ax):
        ...
        ax.set_xlabel('X')
        ax.set_ylabel('Z')
        ax.set_title(f'Поле скоростей вокруг круга. Вид сбоку.')
...
def circle_calculate_slice_Oy_implicit(grid_x, grid_y, coord_x, coord_y, radius):
    # Константы
    U, V = 1.0, 1.0  # Скорости потока
    kv = 0.1  # Кинематическая вязкость
    rho = 1.0  # Плотность
    ...
    def solve_flow(u, v, p, dt, nt):
        for _ in range(nt):
            ...
            for i in range(nx):
                for j in range(ny):
                    u[i, j] = U - i * dt_U  # Граничное условие слева и справа
...
def rectangle_calculate_slice_Oy_explicit(grid_x, grid_y, coord_x, coord_y, length, height):
    # Константы
    U, V = 1.0, 1.0  # Скорости потока
    kv = 0.1  # Кинематическая вязкость
    rho = 1.0  # Плотность
    ...
    # Определение находится ли точка внутри объекта или нет
    def is_inside_rectangle(x, y, cx=int(coord_x), cy=int(coord_y), lR=int(length), hR=int(height)):
        return abs(x - cx) < lR / 2 and abs(y - cy) < hR / 2

    is_inside_object = is_inside_rectangle
    ...
    def solve_flow(u, v, p, dt, nt):
        for _ in range(nt):
            ...
            # Поле скоростей не учитывая давление и плотность
            u[1:-1, 1:-1] = (
                    u_new[1:-1, 1:-1] - dt * (np.round(u_new[1:-1, 1:-1], 6) * (
                    np.round(u_new[1:-1, 1:-1], 6) - np.round(u_new[1:-1, :-2], 6)) / dx +
                                              np.round(v_new[1:-1, 1:-1], 6) * (
                                                      np.round(u_new[1:-1, 1:-1], 6) - np.round(u_new[:-2, 1:-1],
                                                                                                6)) / dy) +
                    kv * dt * ((np.round(u_new[1:-1, 2:], 6) - 2 * np.round(u_new[1:-1, 1:-1], 6) + np.round(
                u_new[1:-1, :-2], 6)) / dx ** 2 +
                               (np.round(u_new[2:, 1:-1], 6) - 2 * np.round(u_new[1:-1, 1:-1], 6) + np.round(
                                   u_new[:-2, 1:-1], 6)) / dy ** 2))

            v[1:-1, 1:-1] = (
                    v_new[1:-1, 1:-1] - dt * (np.round(u_new[1:-1, 1:-1], 6) * (
                    np.round(v_new[1:-1, 1:-1], 6) - np.round(v_new[1:-1, :-2], 6)) / dx +
                                              np.round(v_new[1:-1, 1:-1], 6) * (
                                                      np.round(v_new[1:-1, 1:-1], 6) - np.round(v_new[:-2, 1:-1],
                                                                                                6)) / dy) +
                    kv * dt * ((np.round(v_new[1:-1, 2:], 6) - 2 * np.round(v_new[1:-1, 1:-1], 6) + np.round(
                v_new[1:-1, :-2], 6)) / dx ** 2 +
                               (np.round(v_new[2:, 1:-1], 6) - 2 * np.round(v_new[1:-1, 1:-1], 6) + np.round(
                                   v_new[:-2, 1:-1], 6)) / dy ** 2))
            ...
            for i in range(ny):
                for j in range(nx):
                    if is_inside_object(X[i, j], Y[i, j]):
                        u[i, j] = v[i, j] = 0
    ...
    def plot_flow(u, v, ax):
        ax.clear()
        ax.streamplot(X, Y, u, v, color="blue")
        rectangle = plt.Rectangle((int(coord_x) - int(length) / 2, int(coord_y) - int(height) / 2), int(length),
                                  int(height), color='r',
                                  alpha=0.5)
        ...
        ax.set_xlabel('X')
        ax.set_ylabel('Z')
        ax.set_title(f'Поле скоростей вокруг прямоугольника. Вид сбоку.')
...
def rectangle_calculate_slice_Oy_implicit(grid_x, grid_y, coord_x, coord_y, length, height):
    # Константы
    U, V = 1.0, 1.0  # Скорости потока
    kv = 0.1  # Кинематическая вязкость
    rho = 1.0  # Плотность
...
def circle_streamlines_Oy(grid_x, grid_y, coord_x, coord_y, radius):
    # Константы
    U, V = 1.0, 1.0  # Скорости потока
    kv = 0.1  # Кинематическая вязкость
    rho = 1.0  # Плотность
    ...
    def plot_flow(u, v, ax):
        ...
        ax.set_xlabel('X')
        ax.set_ylabel('Z')
        ax.set_title(f'Поле скоростей возле круга. Вид сбоку.')
...
def rectangle_streamlines_Oy(grid_x, grid_y, coord_x, coord_y, length, width):
    # Константы
    U, V = 1.0, 1.0  # Скорости потока
    kv = 0.1  # Кинематическая вязкость
    rho = 1.0  # Плотность
